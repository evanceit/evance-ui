...
export function useVirtual<T>(props: VirtualProps, items: Ref<readonly T[]>) {
    ...
    const unwatch = watch(hasInitialRender, (v) => {
        ...
        // First render is complete, update offsets and visible items in case our assumed item height was incorrect.
        unwatch();
    ...
    function _calculateVisibleItems() {
        ...
        const end = clamp(
            calculateIndex(endPx) + 1,
            start + 1,
            items.value.length,
        );
        if (
            // Only update the side we're scrolling towards, the other side will be updated incidentally
            (direction !== UP || start < first.value) &&
            (direction !== DOWN || end > last.value)
        ) {
            const topOverflow =
                calculateOffset(first.value) - calculateOffset(start);
            const bottomOverflow =
                calculateOffset(end) - calculateOffset(last.value);
