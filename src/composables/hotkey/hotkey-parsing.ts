...
import { normalizeKey } from "@/composables/hotkey/key-aliases";
 ...
 * e.g. `ctrl+k`, `shift--`
 */
export function splitKeyCombination(
    combination: string,
    isInternal = false,
): { keys: string[]; separators: string[] } {
    const emptyResult = { keys: [], separators: [] };
    if (!combination) {
        if (!isInternal) consoleWarn("Empty hotkey combination provided");
        return emptyResult;
    }
    ...
    const hasInvalidStructure =
        ...
        combination.includes("__") ||
        combination === "+" ||
        combination === "_" ||
        // Ends with a separator that is not part of a doubled literal
        (combination.length > 1 &&
            (combination.endsWith("+") || combination.endsWith("_")) &&
    ...
    if (hasInvalidStructure) {
        if (!isInternal) {
            consoleWarn(
                `Invalid hotkey combination: "${combination}" has invalid structure`,
            );
        }
        return emptyResult;
    }
    ...
    const hasInvalidMinus = keys.some(
        (key) => key.length > 1 && key.includes("-") && key !== "--",
    );
    if (hasInvalidMinus) {
        if (!isInternal) {
            consoleWarn(
...
export function splitKeySequence(str: string): string[] {
    ...
    if (hasInvalidStart || hasInvalidEnd) {
        consoleWarn(
            `Invalid hotkey sequence: "${str}" contains invalid combinations`,
        );
        return [];
    }
    ...
    const areAllValid = collapsed.every(
        (s) => splitKeyCombination(s, true).keys.length > 0,
    );
