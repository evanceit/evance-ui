...
# Advanced control
Use `dialog.create()` to create a new dialog instance which returns an `EvDialogController` offering better access
to the dialog's lifecycle with dialog's `id` and its `open()`, `close(response?)`, and `dismiss()` functions.
...
const { open, close } = dialog.create();
