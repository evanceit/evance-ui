 ...
 * @param props
 */
export function blockScrollStrategy (data: ScrollStrategyData, props: ScrollStrategyProps) {
    ...
    onScopeDispose(() => {
        scrollElements.forEach((el, i) => {
            const x = parseFloat(el.style.getPropertyValue('--ev-body-scroll-x'));
            const y = parseFloat(el.style.getPropertyValue('--ev-body-scroll-y'));
